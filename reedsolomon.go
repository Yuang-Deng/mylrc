...
func (r *reedSolomon) myCodeSomeShards(matrixRows, inputs, outputs [][]byte, outputCount, byteCount int) {
	if len(outputs) == 0 {
		return
	}
...
func (r *reedSolomon) reconstructLocal1(shards [][]byte) error {
	numberPresent := 0
	shardSize := shardSize(shards)
	for i := 0; i < r.DataShards/2; i++ {
		if len(shards[i]) == 0 {
			numberPresent++
		}
	}
	if numberPresent == 1 {
		...
		subMatrixRow := 0
		for matrixRow := 0; matrixRow < r.DataShards/2 && subMatrixRow < r.DataShards/2; matrixRow++ {
...
func (r *reedSolomon) reconstructLocal2(shards [][]byte) error {
	numberPresent := 0
	shardSize := shardSize(shards)
	for i := r.DataShards/2; i < r.DataShards; i++ {
		if len(shards[i]) == 0 {
			numberPresent++
		}
	}
	if numberPresent == 1 {
		...
		subMatrixRow := 0
		for matrixRow := r.DataShards/2; matrixRow < r.DataShards && subMatrixRow < r.DataShards/2; matrixRow++ {
			if len(shards[matrixRow]) != 0 {
				subShards[subMatrixRow] = shards[matrixRow]
				validIndices[subMatrixRow] = matrixRow
				subMatrixRow++
		...
		for subMatrixRow, validIndex := range validIndices {
			for c := 0; c < r.DataShards/2; c++ {
				subMatrix[subMatrixRow][c] = r.m[validIndex][c+(r.DataShards/2)]
			}
		}
